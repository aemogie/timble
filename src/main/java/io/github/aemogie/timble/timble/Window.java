...
import io.github.aemogie.timble.editor.LevelEditorScene;
...
import io.github.aemogie.timble.eventhandlers.WindowResizeListener;
import io.github.aemogie.timble.renderer.*;
...
import io.github.aemogie.timble.renderer.*;
import io.github.aemogie.timble.util.*;
...
public class Window {
	...
	public static Window create(String title) {
		return create(title, PresetsSettings.GREY);
	}
	...
	public void run(Scene scene) {
		...
		Logger.debugLog("Trying to run window loop...");
		windowLoop();
		Logger.debugLog("Successfully exited the window loop!");
		Logger.debugLog("Destroying the window context...");
		windowDestroy();
...
//		glfwWindowHint(GLFW_MAXIMIZED, GLFW_TRUE);
	...
	public void windowLoop() {
		...
		Shader defaultShader = AssetPool.getShader(StringUtils.resourcePath("shaders/Default.glsl"));
		Shader pickingShader = AssetPool.getShader(StringUtils.resourcePath("shaders/Picking.glsl"));
...
//			glViewport(0, 0, width, height);
	...
	public void changeScene(Scene newScene) {
		...
		currentScene.start();
		Logger.debugLog("Successfully changed the scene to: " + currentScene);
	}
	...
	public void setColor(Color color) {
		...
		Logger.debugLog("Successfully set the background color to: " + color);
	}
	//endregion
}